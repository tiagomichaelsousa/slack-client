...
use Slack\Testing\Responses\Fixtures\Conversation\SetConversationPurposeResponseFixture;

test('from', function () {
    $response = SetConversationPurposeResponse::from($fixture = SetConversationPurposeResponseFixture::ATTRIBUTES);

    expect($response)
        ->toBeInstanceOf(SetConversationPurposeResponse::class)
...
test('fake', function () {
    $response = SetConversationPurposeResponse::fake();

    expect($response->channel)
        ->id->toBe(SetConversationPurposeResponseFixture::ATTRIBUTES['channel']['id']);
});
