...
use Slack\Testing\Responses\Fixtures\Conversation\ConversationInfoResponseFixture;

test('from', function () {
    $response = ConversationInfoResponse::from($fixture = ConversationInfoResponseFixture::ATTRIBUTES);
...
test('fake', function () {
    $response = ConversationInfoResponse::fake();

    expect($response->channel)
        ->id->toBe(ConversationInfoResponseFixture::ATTRIBUTES['channel']['id']);
});
