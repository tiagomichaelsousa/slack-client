...
use Slack\Testing\Responses\Fixtures\Conversation\RenameConversationResponseFixture;

test('from', function () {
    $response = RenameConversationResponse::from($fixture = RenameConversationResponseFixture::ATTRIBUTES);
...
test('fake', function () {
    $response = RenameConversationResponse::fake();

    expect($response->channel)
        ->id->toBe(RenameConversationResponseFixture::ATTRIBUTES['channel']['id']);
});
